...
export const PostHighlight = (divID, color1, set) => {
  documentselector = document.getElementById("printAnnotation");
  windowDoc = document.getElementById("printAnnotation");
  divID = divID;
  color = color1;
...
export function selectAndHighlightRange(id, start, end, color1) {
  if (start == end) {
    return false;
  }
  start_gl = start;
...
function makeEditableAndHighlight(colour) {
  ...
  documentselector.execCommand("foreColor", false, "#ffffff");
  ...
  if (!sel.extentNode.parentElement.hasAttribute("onclick")) {
    ...
    sel.extentNode.parentElement.parentElement.style.boxShadow =
      "0px 2px 8px 0px " + color;
  }
  if (!sel.baseNode.parentElement.hasAttribute("onclick")) {
    ...
    sel.baseNode.parentElement.parentElement.style.boxShadow =
      "0px 2px 8px 0px " + color;
  }
...
function highlight(colour) {
  ...
  if (windowDoc.getSelection) {
    ...
    range.execCommand("foreColor", false, "#ffffff");
  }
}
